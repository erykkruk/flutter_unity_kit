# Unity Content Loading and Generation Guide
...
1. [Options Overview](#1-options-overview)
...
8. [Asset Streaming and LOD](#8-asset-streaming-and-lod)
9. [Remote Configuration](#9-remote-configuration)
10. [Hot Content Updates (Without Store Updates)](#10-hot-content-updates-without-store-updates)
11. [AR Foundation](#11-ar-foundation)
12. [Platform Limitations (iOS vs Android)](#12-platform-limitations-ios-vs-android)
...
| **Addressables + CCD** | Yes | No | Yes | Yes | Medium | Main content delivery system |
...
| **AR Foundation** | N/A | No | Yes | Yes | Medium | AR experiences with camera |
...
Unity SceneManager allows loading entire scenes at runtime — either **replacing** the current scene or **additively** (layer on layer). A scene in Unity is a container holding GameObjects, their components, lighting, lightmaps, navmesh, skybox, and other environment settings. A scene = a `.unity` file in the editor.
...
│                                                                       │
│  Use case: transitioning from main menu to gameplay, between levels. │
├──────────────────────────────────────────────────────────────────────┤
...
│                                                                       │
│  Use case: modular design, UI overlay, streaming open world.         │
└──────────────────────────────────────────────────────────────────────┘
```
...
// ═══════════════════════════════════════════════════════
SceneManager.LoadScene("ModelShowroom", LoadSceneMode.Single);
// The entire frame is blocked until the scene is loaded.
// On mobile = UI freeze, possible ANR (Application Not Responding).

// ═══════════════════════════════════════════════════════
// 2. ASYNCHRONOUS — basic (recommended on mobile)
// ═══════════════════════════════════════════════════════
AsyncOperation op = SceneManager.LoadSceneAsync("ModelShowroom", LoadSceneMode.Additive);

// The scene loads in the background — Unity allocates time per frame.
// progress: 0.0 → 0.9 = loading, 0.9 = ready for activation.
op.completed += (asyncOp) => {
    Debug.Log("Scene ModelShowroom loaded!");
};
...
IEnumerator LoadSceneWithProgress(string sceneName)
{
    ...
    op.allowSceneActivation = false; // Wait — don't activate right away
...
IEnumerator UnloadScene(string sceneName)
{
    ...
    // After unloading a scene, assets may still remain in memory!
...
public class SceneController : MonoBehaviour
{
    ...
    {
        // Load the new scene BEFORE unloading the old one — avoids an "empty frame"
...
**Dart (Flutter to Unity via unity_kit bridge):**
...
// Tracking scene loading (progress bar in Flutter)
...
Additive scenes allow building a modular architecture. The key principle: **one persistent scene + swappable modules**.
...
+ Additive scene "ModelCollection":      ← another additive scene
  ├── GridLayout (3D grid)
  ├── ScrollSystem
...
+ Additive scene "ModelCustomizer":      ← customize flow
  ├── CustomizePedestal
  ├── MaterialPicker
  ├── ColorWheel
  ├── CustomizeCamera
...
// The lighting and skybox of the CURRENT active scene are used.
...
### Scene Sources — Where Can Scenes Be Loaded From?
...
| **Addressables Remote** | Same as above, but the group has a remote build path | New scenes without app update |

```csharp
// ═══ From Build Settings ═══
SceneManager.LoadSceneAsync("ModelShowroom", LoadSceneMode.Additive);
...
AssetBundle sceneBundle = AssetBundle.LoadFromFile(
    Path.Combine(Application.persistentDataPath, "scenes/modelshowroom")
);
// Step 2: load the scene from the bundle (by name, not by path)
SceneManager.LoadSceneAsync("ModelShowroom", LoadSceneMode.Additive);
// Step 3: unload the bundle after the scene is loaded
sceneBundle.Unload(false);

// ═══ From Addressables ═══
var sceneHandle = Addressables.LoadSceneAsync(
    "Scenes/ModelShowroom",
    LoadSceneMode.Additive,
    activateOnLoad: true
);
...
**Problem:** Lightmaps, reflection probes, and navmesh are per-scene. With additive loading they can "clash."

**Solutions:**

| Problem | Solution |
|---------|----------|
| Lightmaps from two scenes overlap | Bake lightmaps separately per scene. Each scene has its own LightmapData. Unity automatically manages offsets |
| Two scenes have different skyboxes | Set the skybox on the active scene. `SceneManager.SetActiveScene()` switches the skybox |
| Navmesh doesn't connect between scenes | Use the `NavMeshSurface` component (from AI Navigation package) — builds navmesh at runtime, can combine multiple scenes |
...
| Async loading = zero lag with proper implementation | Large scenes = memory spikes (peak RAM during loading) |
...
| Swap pattern (load new, then unload old) | `Resources.UnloadUnusedAssets()` after unload is slow (50-200ms) |

### Platforms — Details

**iOS:**
- SceneManager works identically in Flutter embed. The Unity player has its own game loop.
- **Peak memory**: During loading of a new scene (before unloading the old one), both scenes are in RAM. On iPhones with 3GB RAM this is critical — plan swaps carefully.
- **allowSceneActivation = false**: The scene is 90% loaded in memory, but objects don't have Awake/Start yet. This is a good moment to show a loading screen.
...
**Flutter-specific:**
- Unity scene state is **invisible** to Flutter — Unity must send a callback message.
...
**Key difference vs scenes:** Scene = entire context (lighting, camera, everything). Prefab = a single object placed into an existing context.
...
var loadHandle = Addressables.LoadAssetAsync<GameObject>("Models/MyModel_001");
...
// Cleanup: Release the prefab when you no longer need more instances
...
// Cleanup: ReleaseInstance destroys the object AND frees the asset
...
// From a local file (previously downloaded)
...
GameObject prefab = bundle.LoadAsset<GameObject>("MyModel_001");
...
GameObject remotePrefab = remoteBundle.LoadAsset<GameObject>("MyModel_001");
Instantiate(remotePrefab);

// Cleanup
bundle.Unload(false); // false = don't destroy loaded assets in memory
...
GameObject prefab = Resources.Load<GameObject>("Models/MyModel_001");
Instantiate(prefab);
...
public class ModelManager : FlutterMonoBehaviour
{
    ...
    private async Task SwapModel(string newToyId, string source, string url = null)
    {
        GameObject oldToy = _currentModel;

        // Load new one (old one still visible)
        await LoadModel(newToyId, source, url);

        // Destroy old one
        if (oldToy != null) Destroy(oldToy);
    }
...
**Dart (Flutter to Unity) — full API:**
...
class ModelController {
  ...
  // Load a model from Addressables
  ...
  // Swap current model for another (smooth swap)
  void swapModel(String newToyId) {
...
|----------|--------|--------|---------|
| Single model preview | `LoadModel` | Addressables | Load into a scene with a pedestal, orbiting camera, lighting |
| Collection (grid) | Loop of `InstantiateAsync` | Addressables | Load thumbnails (LOD 2) onto a grid. Full model only after tap |
| Swipe carousel | `SwapModel` | Addressables | Preload next/previous. Swap without an empty frame |
| NFT from marketplace | `LoadModel(gltf)` | glTF from URL | Model comes as .glb from an API. No prefab in Unity |
| Customization | `SetMaterial` | Addressables | Load a material variant. Change on the renderer |
| Seasonal skin | `SetMaterial` | Remote Config + Addressables | RC specifies the material address, then load from Addressables |
| Collection gallery | Loop load + `RenderTexture` | Addressables | Render model to texture, then show as thumbnail in UI |
...
// 4. Assign to the ShaderWarmup component in the Core scene
```
...
├── RGB24:   50 x 3 x 12 MB = 1800 MB  ← won't fit in RAM
...
Archives of platform-specific assets (models, textures, materials, prefabs, scenes) loaded at runtime. This is a **low-level** system — the foundation on which Addressables is built. Each bundle is a binary file containing serialized Unity assets in the native format for the target platform.

**When to use raw AssetBundles instead of Addressables?**
- When you need full control over packing, versioning, and caching
- When you have an existing pipeline/CDN and don't want to depend on Unity's abstractions
...
│  4. When not needed → Unload bundle                          │
...
{
    // On Android: StreamingAssets is inside JAR (compressed) → need UnityWebRequest
    ...
    // Load prefab from the bundle
    var assetRequest = bundle.LoadAssetAsync<GameObject>("MyModel_001");
...
// ═══════════════════════════════════════════════════════
// 3. MANAGING DEPENDENCIES (manual!)
// ═══════════════════════════════════════════════════════

// Problem: model_rare prefab uses a material from the shared_materials bundle.
// If shared_materials is not loaded → missing material (magenta).

IEnumerator LoadWithDependencies(string bundleName)
{
    // 1. Load the master manifest (contains dependency info)
    ...
    var prefab = mainRequest.assetBundle.LoadAsset<GameObject>("MyModel_001");
...
// Loaded assets (prefabs, textures) STILL REMAIN in memory.
// You can no longer load new assets from this bundle.
bundle.Unload(false);

// Unload(true): release the bundle + ALL loaded assets.
...
| Option | Disk Size | Load Time | RAM During Loading | Usage |
|--------|:-:|:-:|:-:|---------|
...
| **Per-group** | A group of models = one bundle (e.g. "common", "rare") | When models in a group are loaded together |
...
```
Recommended bundle structure for your project:
...
| Advantages | Disadvantages |
|------------|---------------|
| Full control over packing and strategy | Manual dependency management — you must track EVERY one |
...
**Android:**
- `StreamingAssets` is packed inside the APK (in the `/assets/` folder of the JAR) — it is compressed and you **cannot** use `AssetBundle.LoadFromFile()`. It requires `UnityWebRequest` or `BetterStreamingAssets` (asset store).
- **Google Play Asset Delivery (PAD)**: For apps >150MB. Bundles can be delivered as:
  ...
  - `on-demand`: downloaded when the user needs it (ideal for models)
- Scoped Storage (Android 11+): save downloaded bundles to `Application.persistentDataPath` — the only location that doesn't require permissions.
...
- **Cellular download limit**: ~200MB. Apple blocks downloads >200MB over LTE. Make sure per-model bundles are <10MB, or inform the user about needing Wi-Fi.
- **Background download**: iOS does not allow Unity to download in the background. Use native `NSURLSession` with background configuration (requires a native plugin).
- **App Thinning**: Bundles in StreamingAssets are NOT subject to App Thinning (iOS doesn't know what's in them). Size is 1:1.
...
A modern, higher-level asset management system built **on top of** AssetBundles. It uses addresses (strings) for references — automatically managing dependencies, memory, and remote/local loading. **This is the recommended system for new Unity projects.**

### Key Concepts

| Concept | Description |
|---------|-------------|
| **Address** | A string identifying an asset (e.g. `"Models/MyModel_001"`). Independent of the file path |
| **Group** | A set of addresses built into a single AssetBundle. Controls strategy (local/remote) |
| **Label** | A tag on assets. Allows loading groups of assets by tag (e.g. `"rare"`, `"seasonal"`) |
...
| **Content State** | A `.bin` file saving the build state. Critical for content-only updates |
...
│     │   Build Path: RemoteBuildPath                │           │
│     │   Load Path: RemoteLoadPath                  │           │
│     │   Assets: ModelShowroom.unity, AR.unity        │           │
│     └─────────────────────────────────────────────┘           │
│                                                               │
...
│     → checks cache (is the bundle already downloaded)         │
...
│     → if no asset from the bundle is in use                  │
│       → frees the bundle from memory                          │
└──────────────────────────────────────────────────────────────┘
```
...
var handle = Addressables.LoadAssetAsync<GameObject>("Models/MyModel_001");
GameObject prefab = await handle.Task;
GameObject instance = Instantiate(prefab);
// IMPORTANT: handle must be Release()'d when you no longer need more instances

// B. Load + Instantiate in one step
var instance = await Addressables.InstantiateAsync("Models/MyModel_001").Task;
// Cleanup: Addressables.ReleaseInstance(instance) → destroys AND frees
...
IList<GameObject> rareToys = await listHandle.Task;
...
// ... use instance ...
Destroy(instance);
Addressables.Release(handle); // ← free the reference

// EVEN BETTER — InstantiateAsync (auto reference counting):
var instance = await Addressables.InstantiateAsync("Models/MyModel_001").Task;
// ... use instance ...
Addressables.ReleaseInstance(instance); // destroys AND frees reference

// ═══════════════════════════════════════════════════════
// 3. PRELOADING — download to cache without loading into RAM
// ═══════════════════════════════════════════════════════

// Scenario: user is browsing a list of models in Flutter.
// Preload the next 3 models in the background (download bundle, but don't load to RAM).

public class ModelPreloader
{
    private readonly List<AsyncOperationHandle> _downloadHandles = new();

    // Download bundle to cache (disk) — don't load into RAM
    public async Task PredownloadModels(List<string> modelIds)
    {
        foreach (string modelId in modelIds)
        {
            ...
            if (size > 0)
            {
                // Download to cache (don't load into RAM)
    ...
    // Check the total download size for an entire group
...
This is the **most important** Addressables feature for content-driven apps. It allows adding new models, scenes, materials — without submission to the App Store or Google Play.

```csharp
/// <summary>
/// Manages Addressables catalog updates.
/// Call on app start (loading screen).
/// </summary>
public class CatalogUpdater : MonoBehaviour
{
    ...
    {
        // Step 1: Initialize Addressables (if not already initialized)
        ...
        // Step 3: Optionally check the download size
        long totalSize = 0;
        ...
        // Step 4: Download the updated catalog
        ...
        // Step 5: (Optionally) Preload new assets
...
2. Mark the prefab as Addressable: "Models/NewToy_042"
3. Add to group "RemoteToys_Common", label "common"
...
5. Upload to CDN/CCD:
   ...
   └── Only changed bundles (the new model)

6. Users:
   ├── Open app → CatalogUpdater checks hash
   ├── New hash → download new catalog (a few KB)
   ├── "Models/NewToy_042" is now available
   └── When user wants to see it → download the bundle (on demand)
...
3. New app build → generates a NEW .bin → archive the new one
...
2. Create buckets:
   ├── "your-project-android-prod"
   ├── "your-project-ios-prod"
   ├── "your-project-android-staging"
   └── "your-project-ios-staging"
...
| Above 50TB | $0.06/GB |
| Above 500TB | $0.03/GB |
| Formats | .bundle, .gzip, .txt, .json and more |
...
| Releases | Immutable content snapshots. Rollback = change the badge |
...
| **Google Cloud Storage** | ~$0.08/GB | Medium | Good integration with Firebase |
| **Azure Blob + CDN** | ~$0.087/GB | Medium | Enterprise |
| **Firebase Hosting** | Free 10GB/day | Low | Simple setup, limits |
...
string modelAddress = RemoteConfigService.Instance
    .appConfig.GetString("featured_toy_address"); // "Models/LimitedEdition_001"
...
// Shows: what's loaded, reference count, bundles in memory

// ═══ Cache clearing ═══
// Clear the ENTIRE Addressables cache (e.g. after a major update):
Caching.ClearCache();
// Or per bundle:
...
| Advantages | Disadvantages |
|------------|---------------|
| Automatic dependencies — no manual tracking needed | Abstraction adds debugging complexity |
| Reference counting -> no memory leaks (if you Release()) | Learning curve for the pipeline (groups, profiles, labels) |
| Memory management (Release() frees automatically) | Automation doesn't suit ultra-granular control |
| Content update without rebuilding the app! (remote catalog) | `addressables_content_state.bin` must be archived |
| Labels -> load asset groups with a single call | Profiler/Event Viewer requires learning |
| Integration with CCD (zero custom CDN) | GetDownloadSizeAsync is not always accurate |
...
| **.glb** | Binary — everything in a single file | Smaller | Faster (one file) |

**Recommendation: GLB** — one file, faster loading, less overhead.

### Why Is This Important?

| Scenario | Description |
|----------|-------------|
| **NFT from marketplace** | 3D models as NFTs with metadata containing a URL to a .glb |
...
| **Dynamic content API** | Backend returns a URL to a model. App doesn't need to know in advance what it will load |
| **A/B testing visuals** | Different model variants without rebuilding the app |
...
if (success) {
    // Instantiate the MAIN scene from the glTF file
...
// ═══ Cleanup ═══
gltf.Dispose(); // Release native resources (textures, meshes)
```
...
### Full GltfModelLoader Implementation (C#)
...
```
            ...
            // Auto-scale to unified size (item on podium)
...
2. glTFast detects it automatically and decompresses with Burst
...
- **Both**: glTFast with Burst provides **3-5x** better parsing performance on mobile ARM than pure C# (UnityGLTF). Large GLB files (50MB+) should be loaded asynchronously with a progress indicator.
- **Peak memory**: During GLB parsing, the file + decompressed data exist in RAM simultaneously. For a 20MB GLB, peak RAM can be ~60MB. On low-end mobile this is significant.
...
Creating 3D geometry programmatically at runtime — without pre-made assets. You define vertices, triangles, normals, UVs, and colors in C# code. Unity converts this to GPU buffers.

**When does this make sense vs loading pre-made models?**
...
// ===============================================================
// 3. PARAMETRIC OBJECT — cylinder with Flutter control
// ===============================================================
...
### Scenarios for 3D content apps

| Scenario | Description | Complexity |
|-----------|------|-----------|
| **Pedestal/podium** | Procedural pedestal under the item (shape depends on rarity) | Low |
| **Particle trails** | Custom mesh for trail renderer (e.g., sparkle trail behind item) | Medium |
| **Data visualization** | 3D stats chart for the item (bar chart, radar chart) | Medium |
| **Dynamic backgrounds** | Procedural terrain/skybox behind the item | Medium |
| **Customizable accessories** | Parametric hats, glasses on the item (slider-driven) | High |
| **Morphing effects** | Vertex displacement animation (transform item) | High |

### Key APIs

| API | Description | When to use |
|-----|------|-------------|
| `mesh.MarkDynamic()` | Informs GPU that the mesh will be frequently modified | Animated/parametric meshes |
...
- **`MarkDynamic()`**: For meshes changed every few frames. NOT for static ones (wastes GPU memory).
- **iOS (Metal)**: Handles runtime meshes well. Burst compiled generation works on ARM64. Avoid rebuilding meshes every frame if not necessary.
...
- **Colliders**: `MeshCollider` with runtime mesh is expensive. On mobile prefer primitive colliders (Box, Sphere, Capsule) or convex colliders. If you must — `MeshCollider.sharedMesh = mesh` (not every frame!).

---

## 8. Asset Streaming and LOD

### Texture Streaming

Unity has a built-in texture streaming system (Mipmap Streaming) — it loads only the necessary mipmaps to the GPU based on camera distance from the object. This drastically reduces VRAM usage.

```
+------------------------------------------------------------+
|  Mipmap levels (for a 2048x2048 texture):                   |
|                                                             |
|  Mip 0: 2048x2048 = 16 MB (ASTC 4x4)    <-- close to cam  |
...
| Mip Priority (main item) | +2 | +1 | 0 |

**Pop-in problem:** When the camera moves close quickly, the texture may appear blurry for a moment (loading higher mip). Solutions:
- `StreamingController` component on camera — preloads mips in advance
- Higher mip priority on key objects (main 3D items)
- Larger budget (at the cost of RAM)

### LOD (Level of Detail)

`LODGroup` component automatically switches between mesh variants of different detail levels depending on the object's screen size.
...
```csharp
// === Manual LOD setup on runtime-loaded prefab ===
public void SetupLOD(GameObject item, GameObject[] lodMeshes)
{
    var lodGroup = item.AddComponent<LODGroup>();
...
public async Task LoadItemProgressive(string itemId, Transform parent)
{
    // Step 1: Load LOD 2 immediately (500 tris, tiny bundle)
    var lodLow = await Addressables.InstantiateAsync($"Items/{itemId}_LOD2", parent).Task;

    // Step 2: Load LOD 0 in the background (full detail, bigger bundle)
    var lodHigh = await Addressables.InstantiateAsync($"Items/{itemId}_LOD0", parent).Task;
...
public class ContentStreamingManager : MonoBehaviour
{
    ...
    // === Layer 3: On Demand ===
    public async Task<bool> EnsureItemDownloaded(string itemId)
    {
        var sizeHandle = Addressables.GetDownloadSizeAsync($"Items/{itemId}");
        long size = await sizeHandle.Task;
        Addressables.Release(sizeHandle);

        if (size == 0) return true; // Already in cache

        // Inform Flutter about the download need
        NativeAPI.SendToFlutter(JsonUtility.ToJson(new {
            type = "item_download_required",
            itemId = itemId,
            sizeBytes = size
        }));

        // Download
        var downloadHandle = Addressables.DownloadDependenciesAsync($"Items/{itemId}");
    ...
    public async Task PreloadInBackground(List<string> itemIds)
    {
        foreach (string itemId in itemIds)
        {
            ...
            if (size > 0)
            {
                var handle = Addressables.DownloadDependenciesAsync($"Items/{itemId}");
                await handle.Task;
                Addressables.Release(handle);

                // Breathe — don't block main thread
...
Changing game behavior, visuals, and parameters from the server — **without an app update**. Unity offers an official service, **Unity Remote Config** (part of Unity Gaming Services). It works as a key-value store with a rules engine.

**What Remote Config is NOT:**
- NOT for delivering assets (use Addressables for that)
- NOT for changing code (use HybridCLR for that)
- NOT a database (use Cloud Save / your own backend for that)
...
//
// Keys:
// +-- featured_item_id         (string)  = "item_042"
// +-- item_rotation_speed      (float)   = 2.5
// +-- show_particle_effects    (bool)    = true
// +-- seasonal_theme           (string)  = "default"
// +-- lighting_preset          (string)  = "warm"
// +-- max_items_in_view        (int)     = 12
// +-- item_showcase_layout     (json)    = {"rows":3,"cols":4}
// +-- featured_items_list      (json)    = ["item_001","item_042","item_099"]
//
// Rules (conditions):
...
// |   Override: max_items_in_view = 20, show_particle_effects = true
// |
// +-- Rule: "Android Low-End"
// |   Condition: platform == "Android" AND systemMemory < 3000
// |   Override: max_items_in_view = 6, show_particle_effects = false
// |
...
{
    // Cached values
    public string FeaturedItemId { get; private set; }
    public float ItemRotationSpeed { get; private set; }
    ...
    public int MaxItemsInView { get; private set; }
    ...
    private void OnFetchCompleted(ConfigResponse response)
    {
        ...
        FeaturedItemId = config.GetString("featured_item_id", "item_001");
        ItemRotationSpeed = config.GetFloat("item_rotation_speed", 1.5f);
        ...
        MaxItemsInView = config.GetInt("max_items_in_view", 12);
...
// In Dashboard: key "item_showcase_config" = JSON:
// {
//   "layout": "grid",
//   "rows": 3,
//   "cols": 4,
//   "showRarity": true,
//   "sortBy": "rarity",
//   "featuredItems": ["item_001", "item_042", "item_099"]
// }

string jsonConfig = config.GetJson("item_showcase_config");
...
### Use cases for 3D content apps — complete list

| Category | Key | Type | Default | Description |
|-----------|-------|-----|---------|------|
| **Featured** | `featured_item_id` | string | `"item_001"` | Item on the main screen |
| **Featured** | `featured_items_list` | json | `[]` | List of items in showcase |
| **Visual** | `item_rotation_speed` | float | `1.5` | Auto-rotation speed |
...
| **Layout** | `max_items_in_view` | int | `12` | Max items in collection |
| **Layout** | `item_showcase_config` | json | `{...}` | Showcase layout configuration |
...
| **Content** | `new_item_addressable` | string | `""` | Addressable for the new item |
...
class RemoteConfigController {
  ...
  // Listen for config changes from Unity
...
class RemoteConfig {
  ...
  RemoteConfig({
    required this.featuredItemId,
    required this.rotationSpeed,
    required this.showParticles,
    required this.seasonalTheme,
    required this.lightingPreset,
    required this.maxItemsInView,
  });

  factory RemoteConfig.fromJson(Map<String, dynamic> json) {
    return RemoteConfig(
      featuredItemId: json['featuredItemId'] as String? ?? 'item_001',
      ...
      maxItemsInView: json['maxItemsInView'] as int? ?? 12,
    );
  }
}
```
...
| **Own backend** | Server cost | Full control | Must build and maintain |
...
## 10. Hot Content Updates (without Store updates)

### Overview

"Hot update" = changing app content or behavior without submitting to App Store / Google Play. The key question: **what** do you want to update?
...
| **Scripts (Lua, Python)** | xLua, IronPython | RISKY | YES |
| **Full APK patch** | UnityAndroidHotUpdate | N/A | YES |
...
|  +-- Keep addressables_content_state.bin <-- CRITICAL!        |
|  +-- Submit to App Store / Google Play                        |
|                                                               |
|  Step 2: CREATING NEW CONTENT (days/weeks later)             |
|  +-- Artist creates new item in Unity Editor                  |
|  +-- Mark as Addressable: "Items/NewItem_042"                 |
|  +-- Add to group "RemoteItems", label "common"               |
|  +-- Optionally: new materials, animations, scenes            |
|                                                               |
|  Step 3: CONTENT-ONLY BUILD (without rebuilding app!)        |
|  +-- Addressables > Build > Update Previous Build             |
...
|  +-- Done — users will get new content                        |
|                                                               |
|  Step 5: USER EXPERIENCE                                      |
|  +-- User opens app                                           |
|  +-- CatalogUpdater checks hash (a few KB)                    |
|  +-- New hash -> download new catalog (~10-50 KB)             |
|  +-- Now "Items/NewItem_042" is available in the catalog      |
|  +-- When user wants to see it -> download bundle (~2-5 MB)   |
|  +-- Item loaded! No Store visit!                             |
|                                                               |
|  ZERO submissions to App Store / Google Play!                 |
|  ZERO waiting for review (Apple: 24-48h)!                    |
|  ZERO forced updates on users!                                |
+--------------------------------------------------------------+
```

**What CAN be updated via Addressables:**

| Asset | Example | Typical size |
|-------|---------|:-:|
| Prefabs (3D models) | New item with meshes, materials, animations | 2-10 MB |
...
### B. HybridCLR (hot update of C# code)
...
|  All code is compiled. Cannot change without rebuilding.      |
|                                                               |
|  With HybridCLR:                                              |
...
|      -> New logic works WITHOUT rebuilding app!               |
|                                                               |
|  IMPORTANT: hot update assemblies must be declared            |
|  upfront in HybridCLR configuration. You cannot               |
...
- Interpreter (HybridCLR runtime) is **built into the app** -> OK
...
- **Risk**: Apple may reject the app if it detects significant behavior changes after downloading a DLL
- **In practice**: Small hot-fixes (bug fixes) pass review. Major gameplay changes -> risk of rejection
...
| Scripts from server | **RISKY** — violates 3.3.2 depending on Apple's interpretation | OK — no restrictions |
...
+--------------------------------------------------------------+
|  INSTEAD OF:                                                  |
|  Download new C# script -> new item behavior                  |
|                                                               |
|  DO:                                                          |
|  ScriptableObject "ItemBehaviorConfig" in Addressables:       |
|  {                                                            |
...
|  }                                                            |
|                                                               |
|  Fixed C# code in the app handles ALL possible values.        |
...
|  |   -> New items, textures, scenes, animations             |
|  +-- Remote Config                                          |
...
|  +-- Unity CCD or own CDN                                   |
|      -> Hosting bundles + catalog                           |
|                                                             |
...
Unity AR Foundation is a cross-platform AR framework — a shared API for ARKit (iOS) and ARCore (Android). It allows overlaying Unity 3D content on the device's camera feed. Instead of writing separate ARKit/ARCore code, you write once in AR Foundation.

### AR Foundation Architecture

```
+----------------------------------------------------------+
|  AR Foundation (shared API)                               |
...
|  +-- AROcclusionManager — occlusion (real objects in front)|
|                                                           |
...
|            +-- 3D Item (loaded from Addressables)         |
|            |   +-- Shadows on AR plane                    |
...
|                                                           |
|  Communication:                                           |
|  Flutter -> Unity: "PlaceItem", "ScaleItem", "RotateItem" |
|  Unity -> Flutter: "ar_plane_detected", "item_placed",    |
...
| Feature | iOS (ARKit) | Android (ARCore) | Usage in 3D content apps |
|---------|:-:|:-:|---------|
| **Plane detection** | Yes (horizontal + vertical) | Yes (horizontal + vertical) | Place item on floor/table |
| **Image tracking** | Yes (up to 100 ref images) | Yes (up to 20 ref images) | Scan card with item -> show 3D model |
| **Face tracking** | Yes (TrueDepth, 52 blendshapes) | Yes (limited, no depth) | Face filter with item (AR selfie) |
| **Body tracking** | Yes (2D + 3D skeleton) | No | Item mimics user poses |
| **Light estimation** | Yes (directional, ambient, probes) | Yes (ambient, directional) | Realistic lighting of item in AR |
...
| **Anchors** | Yes (persistent!) | Yes (Cloud Anchors) | Save item position -> return later |
| **Occlusion** | Yes (people + LiDAR) | Yes (Depth API, limited devices) | Item behind real object |
| **Environment probes** | Yes | Yes | Environment reflections on item |
| **Object tracking** | Yes (scan 3D object) | No | Replace real object with item |
| **Geo anchors** | Yes (ARKit 6+) | Yes (Geospatial API) | Place item at specific GPS location |
...
/// <summary>
/// Manages AR session and communication with Flutter.
/// Mount on GameObject with ARSession, XROrigin.
/// </summary>
public class ARItemManager : FlutterMonoBehaviour
{
    ...
    // --- Enable AR Session ---
    private async Task EnableAR(string itemId)
    {
        ...
        // Load item model
        _currentItem = await Addressables.InstantiateAsync($"Items/{itemId}").Task;
        _currentItem.SetActive(false); // Hide until user taps

        SendToFlutter("ar_enabled", JsonUtility.ToJson(new {
            itemId = itemId,
            message = "Point the camera at a surface"
        }));
    }
    ...
    private void OnPlanesChanged(ARPlanesChangedEventArgs args)
    {
        if (args.added.Count > 0 && _isPlacementMode)
        {
            SendToFlutter("ar_plane_detected", JsonUtility.ToJson(new {
                ...
                message = "Surface detected! Tap to place item."
            }));
        }
    }

    // --- Place Item at Screen Point (tap) ---
    private void PlaceItemAtScreenPoint(Vector2 screenPoint)
    {
        if (raycastManager.Raycast(screenPoint, _raycastHits, TrackableType.PlaneWithinPolygon))
        {
            ...
            {
                _currentItem.transform.position = hitPose.position;
                _currentItem.transform.rotation = hitPose.rotation;
                ...
                SendToFlutter("ar_item_placed", JsonUtility.ToJson(new {
                    positionX = hitPose.position.x,
                    positionY = hitPose.position.y,
                    positionZ = hitPose.position.z,
                    message = "Item placed! Use gestures to move/rotate."
                }));
            }
        }
    }
    ...
    // --- Scale & Rotate ---
    private void ScaleItem(float scale)
    {
        if (_currentItem != null)
            _currentItem.transform.localScale = Vector3.one * Mathf.Clamp(scale, 0.1f, 3f);
    }

    private void RotateItem(float angle)
    {
        if (_currentItem != null)
            _currentItem.transform.Rotate(Vector3.up, angle);
    }
    ...
    private async Task TakeScreenshot()
    {
        ...
        string path = Path.Combine(Application.persistentDataPath, $"ar_screenshot_{DateTime.Now:yyyyMMdd_HHmmss}.png");
    ...
    private void DisableAR()
    {
        ...
        if (_currentItem != null)
        {
            Addressables.ReleaseInstance(_currentItem);
            _currentItem = null;
        }
    ...
    // --- Payloads ---
    [Serializable] struct ARPayload { public string itemId; }
...
class ARScreen extends StatefulWidget {
  ...
  const ARScreen({required this.bridge, required this.itemId});
...
class _ARScreenState extends State<ARScreen> {
  ...
  void initState() {
    super.initState();
    // Enable AR in Unity
    widget.bridge.send(UnityMessage.command('EnableAR', {'itemId': widget.itemId}));
  ...
  Widget build(BuildContext context) {
    return Stack(
      children: [
        // Unity AR view (full screen — camera + 3D)
        ...
        SafeArea(
          child: Column(
            children: [
              ...
              // Instructions
              if (!_planeDetected)
                _buildInstruction('Point the camera at a flat surface'),
              if (_planeDetected && !_itemPlaced)
                _buildInstruction('Tap the surface to place the item'),
              ...
              if (_itemPlaced)
                Slider(
                  ...
                  max: 3.0,
                  onChanged: (value) {
                    setState(() => _itemScale = value);
                    widget.bridge.send(
                      UnityMessage.to('ARItemManager', 'ScaleItem', {'scale': value}),
                    );
                  },
                ),
            ],
...
### Image Tracking — scan card with item

```csharp
// Scenario: physical card with QR/image -> scan -> show 3D item
...
public class ImageTracker : FlutterMonoBehaviour
{
    ...
    private readonly Dictionary<string, GameObject> _trackedItems = new();
    ...
    private void OnTrackedImagesChanged(ARTrackedImagesChangedEventArgs args)
    {
        ...
        {
            string imageName = trackedImage.referenceImage.name; // e.g., "item_card_001"
            _ = SpawnItemOnCard(imageName, trackedImage.transform);
        ...
        {
            if (_trackedItems.TryGetValue(trackedImage.referenceImage.name, out var item))
            {
                item.transform.position = trackedImage.transform.position;
                item.transform.rotation = trackedImage.transform.rotation;
                item.SetActive(trackedImage.trackingState == TrackingState.Tracking);
            }
        }
    }

    private async Task SpawnItemOnCard(string cardId, Transform anchor)
    {
        string itemId = cardId.Replace("card_", "item_"); // card_001 -> item_001
        var item = await Addressables.InstantiateAsync($"Items/{itemId}", anchor).Task;
        item.transform.localPosition = Vector3.up * 0.1f; // Raise above card
        _trackedItems[cardId] = item;
    }
}
```

### Platforms — details and requirements
...
- **Face tracking**: TrueDepth camera (iPhone X+). 52 blendshapes (ARKit), including jaw, eye, eyebrow, mouth
...
- **List of supported devices**: [developers.google.com/ar/devices](https://developers.google.com/ar/devices) — not all Android phones!
...
- **Geospatial API**: Place content at a specific GPS location (Google Maps Visual Positioning System)

**Flutter-specific:**
- Camera permission: handle in Flutter (package `permission_handler`) BEFORE sending `EnableAR` to Unity
...
- **Battery**: AR session uses a lot of battery (camera + GPU + CPU). Inform user or limit session time

---

## 12. Platform limitations (iOS vs Android)

### iOS — key limitations

| Limitation | Details | Impact on app |
|-------------|-----------|-------------------|
| **No JIT** | AOT mandatory (IL2CPP). No `System.Reflection.Emit`, no dynamic code generation. C# `dynamic` keyword does not work | Test reflection-heavy code on device (Editor uses JIT -> masks problems) |
| **Guideline 3.3.2** | Cannot download executable/interpreted code from server. Assets (models, textures, data) OK | Addressables = OK. HybridCLR/Lua from server = risky. Data-driven design = safe |
| **Generic limitations** | AOT must see all generic instantiations at compile time. Missing generic -> runtime crash (`ExecutionEngineException`) | Add `[Preserve]` attribute or link.xml for reflection-used generics |
| **Privacy Manifest** | Since May 2024 — mandatory manifest declaring API usage. Unity automatically generates base, but custom plugins may require additional declarations | Camera (AR), UserDefaults (cache), File access — declare in manifest |
| **Xcode 16+** | Since April 2025 — build requires Xcode 16, targeting iOS 18 SDK | Update Xcode on CI/CD |
| **App Sandbox** | Only `Application.persistentDataPath` for writing. No file system access outside sandbox | Downloaded bundles, cache, screenshots -> persistentDataPath |
| **Cellular download limit** | ~200MB limit for downloads over LTE/5G. Larger downloads require Wi-Fi or user consent | Bundles per item <10MB. Inform about Wi-Fi for large downloads |
| **Metal only** | OpenGL ES deprecated since iOS 12. Metal API only | Test shaders with Metal backend |
| **Background restrictions** | App in background -> Unity pauses. No processing, no downloading | Bundle downloads must be in foreground (or native NSURLSession background) |
| **App size limit** | App Store: 4GB max. Initial download: cellular limit ~200MB | Use Addressables remote -> small base app |
...
// RISKY — generic with reflection
typeof(List<>).MakeGenericType(someType); // crash if AOT didn't see this type

// WORKS — explicit generics
var list = new List<MyItem>(); // AOT sees type at compile time
...
public class GenericHelper {
    ...
    static void PreserveGenerics() {
        new Dictionary<string, List<MyItem>>();
    }
}
```

### Android — key limitations

| Limitation | Details | Impact on app |
|-------------|-----------|-------------------|
| **Scoped Storage (11+)** | Only app-specific directories without permissions. `Environment.getExternalStorageDirectory()` deprecated | Always use `Application.persistentDataPath` |
| **16KB page size (15+)** | Since November 2025 — Google Play requires 16KB page size support. Requires Unity 6000.0.38+ or newer | Use Unity 6000+ LTS. Test on Android 15 device |
| **StreamingAssets** | Packaged in APK (compressed in JAR). `File.ReadAllBytes()` DOES NOT work | Always use `UnityWebRequest` to read StreamingAssets |
| **Play Asset Delivery** | Base APK limit: 150MB (AAB). Above -> use asset packs (install-time, fast-follow, on-demand) | Large bundles as on-demand PAD |
| **NDK compatibility** | Unity bundles its own NDK. Custom native plugins must be compatible with Unity's version | Check NDK version in unityLibrary gradle |
| **Fragmentation** | GPU: Adreno, Mali, PowerVR, Tegra. RAM: 2-16GB. Android 7.0-15+ | Test on minimum: 2GB RAM + Mali GPU + Android 9 |
| **Vulkan vs OpenGL ES** | Vulkan = newer, faster, but not on all devices | Auto Graphics API -> Unity tries Vulkan, fallback to OpenGL ES 3.0 |
| **64-bit requirement** | Google Play requires 64-bit (arm64) since 2019. 32-bit only = rejection | Unity IL2CPP ARM64 = default. Make sure |
| **Doze mode** | Android 6+: aggressive battery management. App in background -> limited CPU | Background downloads may be throttled |
...
|                                                       |
|  install-time:                                        |
|  +-- Downloaded TOGETHER with APK during installation  |
|  +-- Available immediately after install               |
|  +-- Increases installation size                       |
|  +-- Usage: core assets, shared materials              |
|                                                       |
|  fast-follow:                                         |
|  +-- Downloaded AUTOMATICALLY after install            |
|  +-- Does not block installation                       |
|  +-- Available after a few minutes                     |
|  +-- Usage: initial item collection, base scenes       |
|                                                       |
|  on-demand:                                           |
|  +-- Downloaded ON USER REQUEST                        |
|  +-- Requires API call to download                     |
|  +-- Ideal for content on-demand                       |
|  +-- Usage: individual items, AR assets, seasonal      |
|                                                       |
...
### Shared mobile limitations — details

| Limitation | Details | Mitigation |
|-------------|-----------|-----------|
| **Memory** | Low-end: 2GB RAM total (~300MB for app). High-end: 8GB+ (~800MB for app) | Device tier detection -> adaptive quality |
| **Thermal throttling** | Sustained 100% CPU/GPU -> thermal throttle after 3-5 min. FPS drop 50%+ | Target 60fps with headroom. Don't max out CPU/GPU |
| **Background pause** | Unity pauses in background. `OnApplicationPause(true)` -> stop rendering | Save state. Resume gracefully |
| **Shader compilation** | First render of a new shader variant = 50-200ms stutter | `ShaderVariantCollection.WarmUp()` on startup |
| **Garbage Collection** | Unity GC = stop-the-world. Large allocations -> spike | Object pooling. Avoid allocations in Update(). Incremental GC |
| **Draw calls** | Mobile GPU: sweet spot = 50-100 draw calls/frame | Static/dynamic batching. SRP Batcher. GPU instancing |
| **Overdraw** | Transparent objects = drawing the same pixel multiple times | Minimize transparency. Opaque > transparent |
| **Texture memory** | RGBA32 2048x2048 = 16MB. On mobile that's a lot | ASTC compression. Texture streaming. LOD |
| **Battery drain** | GPU + CPU + Network = fast discharge | Adaptive FPS (30fps in idle, 60fps in interaction) |
...
## 14. Integration with unity_kit

### What we already have in unity_kit?

unity_kit has a complete communication and lifecycle infrastructure — the foundation for all scenarios described in this document.
...
| `UnityMessage` | `src/models/unity_message.dart` | Message model with factory: `.command()`, `.to()` |
...
| `UnityAssetLoader` (abstract) | `src/streaming/` | Abstraction for loading strategy (Addressables / raw bundles) |
...
| `MessageBatcher.cs` | `Scripts/UnityKit/` | Outgoing message batching (C# side) |
...
|-----------|-------------|----------|-------------|
| **`ModelManager.cs`** | Spawn/swap/destroy/preload models. Full implementation from section 3 | P0 | FlutterMonoBehaviour, Addressables |
| **`CatalogUpdater.cs`** | CheckForCatalogUpdates on start, progress -> Flutter. Implementation from section 5 | P0 | Addressables |
| **`SceneController.cs`** | LoadSceneIfNeeded, UnloadIfLoaded, SwapScene. Implementation from section 2 | P0 | FlutterMonoBehaviour, SceneTracker |
| **`GltfModelLoader.cs`** | Wrapper for glTFast: load from URL/file/bytes, cache, normalize, center. Section 6 | P1 | glTFast package |
| **`DeviceTierDetector.cs`** | RAM/GPU/CPU detection -> Low/Mid/High. Quality settings per tier. Section 8 | P1 | - |
| **`RemoteConfigManager.cs`** | UGS init, fetch, apply, forward to Flutter. Section 9 | P2 | UGS packages |
| **`ModelPool.cs`** | Object pooling for scrollable collections. Section 3 | P2 | ModelManager |
| **`ShaderWarmup.cs`** | ShaderVariantCollection.WarmUp() on start. Section 3 | P2 | - |
| **`ARModelManager.cs`** | AR session, plane detection, model placement, screenshot. Section 11 | P3 | AR Foundation |
| **`ImageTracker.cs`** | Scan card -> show model. Section 11 | P3 | AR Foundation |

**Dart (Flutter) — missing elements:**

| Component | Description | Priority |
|-----------|-------------|----------|
| **`ModelController`** | High-level API: loadModel, swapModel, preload, setAnimation, setMaterial + streams | P0 |
| **`RemoteConfigController`** | Listen for remote_config_updated, expose config values | P2 |
| **`ARController`** | enableAR, disableAR, placeToy, scaleToy, screenshot + streams | P3 |
| **`ContentDownloadController`** | Manage download UI: progress, size, confirmation dialogs | P1 |

### How individual scenarios map to unity_kit
...
|                                                               |
|  Load glTF from URL              bridge.send() -> GltfModelLoader|
...
|                                                               |
|  AR mode                         bridge.send() -> ARModelManager|
|  +-- Dart: UnityMessage.command()  -> AR Foundation APIs       |
|  +-- Dart: messageStream          <- "ar_toy_placed"          |
|                                                               |
...
class _ModelViewerScreenState extends State<ModelViewerScreen> {
  ...
  late final ModelController _toyController;

  bool _isReady = false;
  bool _isToyLoaded = false;
  ...
  void initState() {
    ...
    _toyController = ModelController(_bridge);
    _initializeUnity();
  }
  ...
  void _onMessage(UnityMessage msg) {
    switch (msg.type) {
      ...
      case 'remote_config_updated':
        // Config updated — Unity already applied settings
        break;
  ...
  void _loadModel() {
    if (widget.modelSource == 'gltf' && widget.glbUrl != null) {
      _toyController.loadModelFromUrl(widget.modelId, widget.glbUrl!);
    } else {
      _toyController.loadModel(widget.modelId);
    }
  }
              ...
              child: Row(
                ...
                children: [
                  _ActionButton(
                    ...
                    onPressed: () => _toyController.setTransform(ry: -45),
                  ),
                  _ActionButton(
                  ...
                  _ActionButton(
                    ...
                    onPressed: () => _toyController.setAnimation('dance'),
                  ),
                  _ActionButton(
                    icon: Icons.color_lens,
                    onPressed: () => _toyController.setMaterial('Materials/Holographic'),
                  ),
                  _ActionButton(
...
- [Addressables Content Catalog Refresh (2024)](https://www.kittehface.com/2024/01/unity-addressables-content-catalog.html)
